...
async function ensureFirebaseProjectAccess(rl, firebaseCliAvailable, firebaseProjectId, appName) {
  ...
  let resolvedProjectId = firebaseProjectId;

  while (true) {
    const projectIds = parseFirebaseProjects(captureCommand('firebase', ['projects:list', '--json']));
    ...
    console.log(
      `\nFirebase project "${resolvedProjectId}" was not found in your accessible projects. Attempting to create it now.`,
    );

    try {
      await runCommand('firebase', ['projects:create', resolvedProjectId, '--display-name', appName]);
      return {
        firebaseProjectId: resolvedProjectId,
        requiresInit: true,
      };
      ...
      const tryAnotherProject = await promptYesNo(
        rl,
        `Failed to create or access Firebase project "${resolvedProjectId}". Enter a different project ID`,
        true,
      );

      if (!tryAnotherProject) {
        throw new Error(
          `Failed to get Firebase project ${resolvedProjectId}. Please make sure the project exists and your account has permission to access it.`,
        );
      }

      resolvedProjectId = sanitizePackageName(
        await promptText(rl, 'New Firebase project ID', 'my-firebase-project'),
      );
    }
  }
...
export async function runScaffolder() {
  ...
  try {
    ...
    const firebaseProjectSelection = await resolveFirebaseProject(
      rl,
      firebaseCliAvailable,
      cliOptions.firebaseProjectId,
    );
    const { firebaseProjectId, requiresInit } = await ensureFirebaseProjectAccess(
      rl,
      firebaseCliAvailable,
      firebaseProjectSelection.firebaseProjectId,
      displayName,
    );
