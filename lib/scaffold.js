...
async function runWebsiteAgent(targetDirectory) {
  ...
  try {
    await runCommand('agent', ['--yolo', '--sandbox', 'disabled', '--approve-mcps', 'Run this website'], { cwd: targetDirectory });
  } catch {
    console.warn('\nFailed to run `agent --yolo --sandbox disabled "Run this website"` automatically.');
  }
}
