...
async function resolveCloudflareTarget(rl, wranglerCliAvailable, presetProjectName, presetAccountId) {
  ...
  if (!wranglerCliAvailable) {
    ...
    const cloudflareAccountId = accountIdFromPreset || (await promptText(rl, 'Cloudflare account ID', ''));
    return {
      cloudflareProjectName,
      cloudflareAccountId,
    };
  ...
  const hasPresetAccount = accountIdFromPreset && availableAccountIds.includes(accountIdFromPreset);

  let cloudflareAccountId = accountIdFromPreset;

  if (!hasPresetAccount) {
    if (accounts.length === 1) {
      cloudflareAccountId = accounts[0].id;
    } else if (accounts.length > 1) {
      const selectedAccount = await promptSelect(
        rl,
        'Choose the Cloudflare account for Cloudflare Pages deploys',
        accounts.map((account) => ({
          label: `${account.name} (${account.id})`,
          value: account.id,
        })),
      );
      cloudflareAccountId = selectedAccount.value;
    } else {
      cloudflareAccountId = await promptText(rl, 'Cloudflare account ID', accountIdFromPreset);
    }
  }

  if (!cloudflareAccountId) {
    cloudflareAccountId = await promptText(rl, 'Cloudflare account ID', '');
  }
...
export async function runScaffolder() {
  ...
  try {
    ...
    let shouldConfigureCloudflareDeployCredentials = false;
    let cloudflareApiToken = process.env.CLOUDFLARE_API_TOKEN || '';

    if (shouldSetupGitHub) {
      const defaultRepoSlug = `JMRSquared/${cloudflareProjectName}`;
      repoSlug = await promptText(rl, 'GitHub repository (owner/repo)', defaultRepoSlug);
      shouldConfigureCloudflareDeployCredentials = await promptYesNo(
        rl,
        `Configure Cloudflare deploy credentials in GitHub (${toCloudflareApiTokenSecretName(cloudflareProjectName)} and ${toCloudflareAccountIdVariableName(cloudflareProjectName)})`,
        true,
      );

      if (shouldConfigureCloudflareDeployCredentials && !cloudflareApiToken) {
        cloudflareApiToken = await promptText(rl, 'Cloudflare API token', '');
      }
    }
    ...
    if (shouldSetupGitHub && repoSlug) {
      ...
      if (shouldConfigureCloudflareDeployCredentials) {
        if (!cloudflareApiToken) {
          console.warn(
            '\nCloudflare API token is empty, so the GitHub Actions deploy secret could not be created automatically.',
          );
        } else {
          await runCommand(
            'gh',
