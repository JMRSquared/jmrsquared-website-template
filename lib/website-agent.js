...
3. Read \`AGENTS.md\`, \`README.md\`, \`package.json\`, \`demo-meta.json\`, \`src/\`, and deploy/config files.
...
| Release Engineer | Commits, pushes to \`main\`, deploy verification, \`demo-meta.json\` accuracy |
...
- HARD REQUIREMENT: keep \`demo-meta.json\` and the demo takedown workflow intact and accurate.
...
7. Takedown anytime on request; auto take-down in **2 weeks**.
...
Also open (or update) a tracking issue titled \`Demo takedown: ${appName} by <takedownAt from demo-meta.json>\` so humans can monitor the scheduled removal.
...
- \`demo-meta.json\` accurate; takedown workflow present.
...
- Takedown tracking issue exists for the scheduled date.
...
- Never remove the demo banner, attribution, motion/3D stack, or takedown automation.
