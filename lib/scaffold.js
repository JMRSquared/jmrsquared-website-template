...
async function runWebsiteAgent(targetDirectory) {
  if (!commandExists('claude')) {
    return;
  }

  const projectBriefPath = path.join(targetDirectory, 'project-brief.md');
  let projectBrief = '';

  try {
    projectBrief = await readFile(projectBriefPath, 'utf8');
  } catch {
    // No project brief provided
  }

  const agentPrompt = projectBrief
    ? `Build the website using this project brief:\n\n${projectBrief}`
    : 'Build this website';

  try {
    await runCommand('claude', ['--dangerously-skip-permissions', agentPrompt], { cwd: targetDirectory });
  } catch {
    console.warn('\nFailed to run `claude --dangerously-skip-permissions` automatically.');
  }
}
...
export async function runScaffolder() {
  ...
  try {
    ...
    const projectBriefInput = await promptText(
      rl,
      'Project brief (paste .md content or URL to .md file)',
      '',
    );
    ...
    const replacements = {
      ...
      '__SITE_DESCRIPTION__':
        `${displayName} is a mobile-first website built with React, Vite, Tailwind CSS, and ${deployProviderName}.`,
    ...
    await runWebsiteAgent(resolvedTargetDirectory);
  } finally {
    if (!promptsClosed) {
      rl.close();
    }
