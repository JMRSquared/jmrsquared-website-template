...
function parseCliArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === '--template') {
      ...
      options.templatePassthroughArgs = argv.slice(index + 2);
      return options;
    }

    if (argument.startsWith('--template=')) {
      options.template = argument.slice('--template='.length);
      options.templatePassthroughArgs = argv.slice(index + 1);
      return options;
    }
...
async function runWebsiteAgent(targetDirectory) {
  ...
  try {
    await runCommand('claude', ['--dangerously-skip-permissions', 'Run this website'], { cwd: targetDirectory });
  } catch {
    console.warn('\nFailed to run `claude --dangerously-skip-permissions "Run this website"` automatically.');
  }
}
